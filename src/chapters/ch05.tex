% REVIEW-ID: logic.ch05.standard-propositional-calculus
% REVIEW-TITLE: 標準命題算
...
\begin{sidebox}[TBred]{対象言語とメタ言語}
  \(p,q,r\) は SPC 内部で用いる命題記号である。一方，\(\alpha,\beta,\gamma\) は任意の論理式を指すために説明側で用いるメタ変項である。例えば「\(\alpha\) が論理式なら \(\sim\alpha\) も論理式」と書くとき，\(\alpha\) 自体が SPC の原始記号というわけではない。
\end{sidebox}
\[
  \text{標準命題算}
  \quad
...
\begin{sidebox}[TBred]{要点}
  ...
  なお，論理式を読み解く際の演算子の優先順位は，強い順に否定 \(\sim\)，連言 \(\land\)，選言 \(\lor\)，含意 \(\supset\)，同値 \(\equiv\) である。
\end{sidebox}
